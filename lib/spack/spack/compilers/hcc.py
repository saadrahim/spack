...
class Hcc(Compiler):
    ...
    print("Helo")
    # hcc has support for using different fortran compilers with the
    # hcc executable.
    @property
    def link_paths(self):
        # hcc links are always the same
        link_paths = {'cc': 'hcc/hcc',
                      'cxx': 'hcc/hcc++'}
    ...
    def cxx11_flag(self):
        if self.is_apple:
            # Adapted from CMake's Applehcc-CXX rules
            # Spack's Applehcc detection only valid from Xcode >= 4.6
    ...
    def cxx14_flag(self):
        if self.is_apple:
            # Adapted from CMake's rules for Applehcc
    ...
    required_libs = ['libhcc']

    @classmethod
    @llnl.util.lang.memoized
    def default_version(cls, comp):
        """The ``--version`` option works for hcc compilers.
        On most platforms, output looks like this::

            hcc version 3.1 (trunk 149096)
            Target: x86_64-unknown-linux-gnu
            Thread model: posix

        On macOS, it looks like this::

            Apple LLVM version 7.0.2 (hcc-700.1.81)
    ...
    def extract_version_from_output(cls, output):
        ...
        match = re.search(
            ...
            r'^Apple (?:LLVM|hcc) version ([^ )]+)|'
            # Normal hcc compiler versions are left as-is
            r'hcc version ([^ )]+)-svn[~.\w\d-]*|'
            r'hcc version ([^ )]+)-[~.\w\d-]*|'
            r'hcc version ([^ )]+)',
            output
        )
    ...
    def fc_version(cls, fc):
        # We could map from gcc/gfortran version to hcc version, but on macOS
        # we normally mix any version of gfortran with any version of hcc.
        if sys.platform == 'darwin':
            return cls.default_version('hcc')
        else:
    ...
    def setup_custom_environment(self, pkg, env):
        ...
        """
        super(hcc, self).setup_custom_environment(pkg, env)
        ...
        if not self.is_apple or not pkg.use_xcode:
            ...
            # with hcc. Those point to Spack's compiler wrappers and
            # consequently render MPI non-functional outside of Spack.
            return

        print ("saad")        # Use special XCode versions of compiler wrappers when using XCode
        # Overwrites build_environment's setting of SPACK_CC and SPACK_CXX
        xcrun = Executable('xcrun')
        xcode_hcc = xcrun('-f', 'hcc', output=str).strip()
        xcode_hccpp = xcrun('-f', 'hcc++', output=str).strip()
        env.set('SPACK_CC', xcode_hcc, force=True)
        env.set('SPACK_CXX', xcode_hccpp, force=True)
